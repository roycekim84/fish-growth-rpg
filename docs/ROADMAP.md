...
## M14 — Deep Sea Ecology `진행 중`

목표: 심해가 얕은 바다와 다른 위험도와 탐험 경로를 가진 첫 번째 확장 지역으로 느껴진다.

- [x] 지역별 NPC 스폰 수 데이터 지원
- [x] 심해의 포식자 밀도 증가 및 개체군 전환
- [x] 심해 퀘스트 보상으로 사냥 물고기 해금
- [x] 사냥 물고기의 심해 감각으로 여는 메아리 해구 경로
- [x] 심해 전용 환경 위험 또는 상호작용 1개
- [ ] 심해 전용 신규 종과 픽셀 에셋
- [ ] 웹 알파 플레이테스트 밸런스 조정

완료 조건: 심해에서 기존 지역과 다른 생태 압박을 느끼고, 퀘스트 보상 종으로 새 경로를 여는 경험이 성립한다.
