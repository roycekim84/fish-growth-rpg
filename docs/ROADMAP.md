...
- SmallFish, PufferFish, HunterFish
- Wander/Flee/Chase 상태
- 감지 범위와 경계 회피
- 종별 최대 개체 수
- 안전 스폰 거리와 재생성
- 화면 밖 업데이트 비용 조정
...
1. NPC 런타임 모델과 상태 머신 구현
2. SmallFish, PufferFish, HunterFish 외형과 이동 속도 연결
3. Wander/Flee/Chase 전환 규칙 구현
4. 종별 최대 개체 수와 안전 스폰 구현
5. 화면 밖 AI 업데이트 주기 최적화
6. 장시간 개체 수 유지 테스트
